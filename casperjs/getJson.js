...
casper.then(function() {
	...
	var i=0;
	//this.each(sessions,function(self,session){
	all_confs = this.evaluate(function() {
			...
			function conference(id,name,date,salle,detail,conferenciers) {
				var img_pattern = /<img.+?src=[\"'](.+?)[\"'].*?>/;
				this.id = id;
				this.name = sanitize(name);
			...
			$( ".session" ).each(function( index,self ) {
				...
				name = $(self).find("h3").text();//self.find("h3").text());
				date = $(self).find(".horaire").text();
				salle = $(self).find(".salle").text();
				detail = $(self).find(".abstract").text();
				...
				conf = new conference(id,name,date,salle,detail,conf_conferenciers)
				myTab.push(conf);
			});
